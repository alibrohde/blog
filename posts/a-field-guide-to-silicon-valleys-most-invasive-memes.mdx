...
AI ability is not a single axis like human intelligence: a model can be a star math student and miss a question most people would find laughably simple. Coined by Andrej Karpathy in 2024 to argue that human comparisons are the wrong frame, since in people most knowledge and problem-solving capabilities are correlated and rise together from childhood to adulthood, while in AI they are uncorrelated across tasks. Shows up in the gap between models scoring five of six at the International Math Olympiad and the same systems telling a user to walk fifty meters to a repair shop instead of driving, and it has shifted the labor debate from "is AI smarter than humans" to "which jagged peaks happen to overlap a given job."
